...
namespace CarsLibrary.UI.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            ApplicationUser user = userMgr.Find(model.Email, model.Password);
            if (user == null)
            {
                ModelState.AddModelError("","Invalid email or password");
                return View(model);
            }
            var currentRole = roles.Where(role => role.Users.Any(u => u.UserId == user.Id)).Select(r => r.Name)
                .FirstOrDefault().ToString();

            if (currentRole.Equals("disabled"))
            {
                ModelState.AddModelError("", "Shouldn't have gone and got disabled!");
                return View(model);
            }
            //}
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { Email = model.Email, PasswordHash = model.Password, UserName = model.Email, LastNameId = model.LastName, FirstNameId = model.FirstName };
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        RoleManager.Create(new IdentityRole("disabled"));
                    }
                    if (model.SelectRole == "admin")
                    {
                        UserManager.AddToRole(user.Id, "admin");
                    }
                    if (model.SelectRole == "sales")
                    {
                        UserManager.AddToRole(user.Id, "sales");
                    }
                    if (model.SelectRole == "disabled")
                    {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                // var callbackUrl = Url.Action("ChangePassword", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);		
